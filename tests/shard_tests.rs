...
use common::{TestContext, verify_processing_complete};
...
async fn test_multiple_shard_processing() -> Result<()> {
    ...
    for i in 1..=3 {
        ctx.client.mock_get_iterator(Ok(format!("iterator-{}", i))).await;
...
async fn test_shard_reshard_handling() -> Result<()> {
    ...
    ctx.client.mock_get_iterator(Ok("iterator-1".to_string())).await;
    ctx.client.mock_get_records(Ok((
    ...
    for suffix in ["1", "2"] {
        ctx.client.mock_get_iterator(Ok(format!("iterator-1-{}", suffix))).await;
        ctx.client.mock_get_records(Ok((
            TestUtils::create_test_records(2),
...
async fn test_checkpoint_recovery_after_failure() -> Result<()> {
    ...
    ctx.client.mock_get_iterator(Ok("test-iterator".to_string())).await;
    ...
    ctx.client.mock_get_records(Ok((test_records.clone(), None))).await;
...
async fn test_iterator_expiration_recovery() -> Result<()> {
    ...
    ctx.client.mock_get_iterator(Ok("iterator-1".to_string())).await;
    ctx.client.mock_get_records(Err(anyhow::anyhow!("Iterator expired"))).await;

    // Second attempt succeeds
    ctx.client.mock_get_iterator(Ok("iterator-2".to_string())).await;
...
async fn test_concurrent_shard_processing_limits() -> Result<()> {
    ...
    ctx.processor.set_pre_process_delay(Some(Duration::from_millis(200))).await;
...
async fn test_graceful_shutdown_with_pending_records() -> anyhow::Result<()> {
    ...
    processor_clone.set_pre_process_delay(Some(Duration::from_millis(500))).await;
    ...
    match result {
        Ok(_) => {
            ...
            assert!(processed_count < 2, "Should not process all records due to shutdown");
            Ok(())
        }
        Err(e) => {
